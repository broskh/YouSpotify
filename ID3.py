...
class ID3:
    ...
    def __str__(self):
        if self.has_tag:
            ...
            return "File   : %s\nTitle  : %-30.30s  Artist: %-30.30s\nAlbum  : %-30.30s  Track : %s  Year: %-4.4s\nComment: %-30.30s  Genre : %s (%i)" % (
            self.filename, self.title, self.artist, self.album, track, self.year, self.comment, genre, self.genre)
        else:
    ...
    def __setattr__(self, name, value):
        if name in ['title', 'artist', 'album', 'year', 'comment',
            ...
            if name == 'track':
                ...
                self.__dict__['d'][str.upper(name)] = self.tupleize(value)
